...
function EditRecipe() {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    setLoading(true);
    axios.put(`${process.env.REACT_APP_API_URL}/api/recipes/${id}`, form)
      .then(() => {
        setLoading(false);
        navigate('/');
      })
      .catch(err => {
        setLoading(false);
        const errorMsg = err.response?.data?.message || err.message || 'Failed to update recipe. Please try again.';
        setError(errorMsg);
    ...
    <div className="add-recipe-page">
      <div className="add-recipe-container">
        <div className="add-recipe-header">
          <h2>Edit Recipe</h2>
          <p>Update the details below and save changes to your recipe.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="name" placeholder="e.g. Spaghetti Carbonara" value={form.name} onChange={handleChange} required />
            </div>
            <div className="form-group">
          ...
          <div className="form-group">
            ...
            <textarea name="ingredients" placeholder="List your ingredients here, one per line" value={form.ingredients} onChange={handleChange} required />
          </div>
          <div className="form-group">
            <label>Instructions</label>
            <textarea name="instructions" placeholder="Write your step by step cooking instructions here" value={form.instructions} onChange={handleChange} required />
          </div>
